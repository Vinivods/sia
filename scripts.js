// modal
const modalElement = document.getElementById('cnpjModal');
        ...
        fetch(`https://api.opencnpj.org/${cnpj}`)
            ...
            .then(data => {
                if (data.erro) {
                    ...
                    document.getElementById('status').value = data.situacao_cadastral;

                    // endereço cobrança
                    document.getElementById('cep_cobranca').value = data.cep;
                    document.getElementById('logradouro_cobranca').value = data.logradouro;
                    document.getElementById('numero_cobranca').value = data.numero;
                    document.getElementById('complemento_cobranca').value = data.complemento;
                    document.getElementById('municipio_cobranca').value = data.municipio;
                    document.getElementById('uf_cobranca').value = data.uf;

                    // porte
                    if (data.porte_empresa == 'Demais') {
                        document.getElementById("grandeporte").checked = true;
                    } else if (data.porte_empresa == 'medio') {
                        document.getElementById("pequenoporte").checked = true;
                    } else {
                        document.getElementById("microporte").checked = true;
                    }

                    // administrador
                    // alert(data.QSA[0])
                    
                    const listaSocios = data.QSA;

                    // alert(listaSocios);

                    // 2. Use o .find() no array completo.
                    // Ele vai testar o item 0, se não for o que quer, testa o 1, o 2... até achar.
                    const administradorFisico = listaSocios.find(socio => 
                    socio.qualificacao_socio === "Administrador" && 
                    socio.identificador_socio === "Pessoa Física"
                    );

                    // 3. Agora você usa o resultado
                    if (administradorFisico) {
                    console.log("Encontrei:", administradorFisico.cnpj_cpf_socio);
                    } else {
                    console.log("Nenhum administrador pessoa física nesta empresa.");
                    }
                    //Saída: "CLAUDIO ROBERTO LINDENMEYER FILHO"
                    document.getElementById("nome_administrador").value = administradorFisico.nome_socio;
                    document.getElementById("cpf_administrador").value = administradorFisico.cnpj_cpf_socio;
...
function adicionarLinha() {
    // const area = document.getElementById("area_regulada").value;
    const area = document.getElementById("area_selecionada").value;
    const codigo = document.getElementById("cod_nfi").value;
    ...
    document.getElementById('area_selecionada').value = "";
    document.getElementById('cod_nfi').value = "";
...
function selecionarAreaRegulada() {
    // Pegamos o elemento que disparou o clique (o item da lista)
    const itemClicado = event.target;

    // Verificamos se o que foi clicado é realmente um link do dropdown
    if (itemClicado.classList.contains('dropdown-item')) {
        
        // Pegamos o texto dentro do link
        const valorSelecionado = itemClicado.innerText;

        // Exibe o alerta para teste
        // alert("Você selecionou: " + valorSelecionado);
        document.getElementById('area_selecionada').value = valorSelecionado;
        // document.getElementById('area_selecionada').innerText = valorSelecionado;
    }
}